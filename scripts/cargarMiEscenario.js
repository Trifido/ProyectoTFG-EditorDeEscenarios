...
function cargarSala( objJson ){
	...
	svgroom = new Room(nombre, pathLocation, pixLargo, pixAncho, ancho, largo, pathX3dSin, pathX3d, pathObj);
	...
	interpolador= new InterpolarCoords( pixAncho, pixLargo, ancho, largo );
	RoomInit = true;
	sceneName = nombre;
}
...
function cargarCuadros( objJson ){
	...
	for(i=0; i<size; i++){
		...
		var alturaCuadro = objJson.scene.cuadros[i].alturaCuadro;

		LoadPictureCanvas(posX, posY, coordX, coordY, cx, cy, rotation, alturaCuadro);
	}
