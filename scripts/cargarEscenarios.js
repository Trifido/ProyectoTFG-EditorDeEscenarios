...
function loadCameraWebVR(JSONEscenario){
	texto= "";
	texto += "<navigationInfo  headlight=\"true\" avatarSize=\'" + 0.1 + " " + JSONEscenario.scene.camera.y + " " + JSONEscenario.scene.camera.salto + "\' type=\'\"game\"\' speed=\"1\" transitiontime=\"1\" transitiontype=\"LINEAR\"></navigationInfo>\n";
...
function initX3DSceneWeb(JSONEscenario){
    ...
    for(var i=0; i<numPictures; i++){
        ...
        texto+='\t\t\t\t\t</Appearance>';
        texto+='\t\t\t\t\t<Box DEF=\"box\" size=\'' + JSONEscenario.scene.cuadros[i].largo + ' ' + JSONEscenario.scene.cuadros[i].alto + '0.025\' />';
        texto+='\t\t\t\t</Shape>';
        texto+='\t\t\t</Transform>\n';
        texto+='\t\t</Transform>\n\n';
    }
