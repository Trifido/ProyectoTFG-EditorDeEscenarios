...
Menu.prototype.loadRotate= function(){
	if(!this.loaded){
		var rotate = document.createElementNS('http://www.w3.org/2000/svg','image');
		rotate.setAttributeNS('http://www.w3.org/1999/xlink','href', './img/submenu/rotate.png');

		rotate.setAttribute('id', 'rotate');
		rotate.setAttributeNS(null, 'nombre', 'rotate');

		this.ancho= parseFloat(this.objDin.ancho);//*0.5;
		this.alto= parseFloat(this.objDin.alto);//*0.5;
		rotate.setAttributeNS(null,'height',this.alto);
		rotate.setAttributeNS(null,'width', this.ancho);
		
		this.despX= (parseFloat(this.objDin.x) + this.ancho*0.5);
		this.despRotY= (parseFloat(this.objDin.z) + this.alto*0.5);
		rotate.setAttributeNS(null,'x',this.despX);
		rotate.setAttributeNS(null,'y',this.despRotY);

		rotate.setAttributeNS(null, 'visibility', 'visible');

		document.getElementById(this.objDin.clase+'_level').appendChild(rotate);
	}
}
