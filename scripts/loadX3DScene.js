...
function initX3DScene( tipo ){
    ...
    for(var i=0; i<nelements; i++){
        ...
        if(elemento.id != 'delete' && elemento.id != 'rotate'){
            ...
            if(objDin.pedestalAlto>0){
                ...
                texto+='\t\t\t\t\t\t<ImageTexture  url="./img/texturaPedestal.jpg"><ImageTexture/>';
    ...
    for(var i=0; i<nelements; i++){
        ...
        if(elemento.id != 'delete' && elemento.id != 'rotate'){
            ...
            var cambioX, cambioZ;
            if(interpolador.anchoSala < 10)
                cambioX = interpolador.getX();// * 0.1;
            else
                cambioX = interpolador.getX();

            if(interpolador.largoSala < 10)
                cambioZ = interpolador.getZ();//* 0.1;
            else
                cambioZ = interpolador.getZ();

            xmlhttp.open("GET", "./php/iniciarPuntoCuadro.php?posX=" + cambioX + "&posY=" + objDin.alturaCuadro + "&posZ=" + cambioZ + "&rotCuadro=" +  objDin.rotCuadro, false);
            xmlhttp.send();

            var arrayCoords = coordPicture.split(' ');

             if(interpolador.anchoSala < 10)
                cambioX = arrayCoords[0] ;//* 10;
            else
                cambioX = arrayCoords[0];

            if(interpolador.largoSala < 10)
                cambioZ = arrayCoords[2]; // * 10;
            else
                cambioZ = arrayCoords[2];

            coordPicture = " " + cambioX + " " + arrayCoords[1] + " " + cambioZ + " ";
            ...
            texto+='\t\t\t\t\t</Appearance>';
            texto+='\t\t\t\t\t<Box DEF=\"box\" size=\'' + objDin.largo + ' ' + objDin.alto + '0.015\' />';
            texto+='\t\t\t\t</Shape>';
            texto+='\t\t\t</Transform>\n';
            texto+='\t\t</Transform>\n\n'; 
