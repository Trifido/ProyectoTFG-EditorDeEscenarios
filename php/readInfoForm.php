<?php
    ...
    if($typeObject == "sculpture"){
        ...
        $query = ' SELECT Cronologia, Tecnica, Informacion, Alto, Largo, AlturaCuadro FROM ' . $tabla . ' WHERE NombreUsuario = \'' . $_SESSION["username"] . '\' AND ' . $AtribNombre . ' = \'' . $nombreBD . '\'';
    }
    else{
        $tabla = "Salas";
        $tabla2 = "ImgSala";
    ...
    if($typeObject != "room"){
        ...
        if($typeObject == "sculpture")
            ...
            $Alto = $array["Alto"];
            $Largo = $array["Largo"];
            $alturaCuadro = $array["AlturaCuadro"];
            $response = array('cronologia'=>$cronologia, 'tecnica'=>$tecnica, 'info'=>$info, 'AnchoPx'=>$AnchoPx, 'AltoPx'=>$AltoPx, 'Alto'=>$Alto, 'Largo'=>$Largo, 'alturaCuadro'=>$alturaCuadro);
        }
    }
    else{
        $result = sqlite_query($db, $query);
