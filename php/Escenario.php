		...
		function __construct( $path_x3d ) {
	  		...
	  		foreach ($xml->Scene->Transform as $node) {
				...
				for($i=0; $i+3<count($coords); $i+=3){
					...
					if((float) $coords[$i+1] < 0)
						$xcoord = ((float) $coords[$i+1]) * -1;
					else
						$xcoord = (float) $coords[$i+1];

					if((float) $coords[$i] < 0)
						$zcoord = ((float) $coords[$i]) * -1;
					else
						$zcoord = (float) $coords[$i];

					if((float) $coords[$i+2] < 0)
						$ycoord = ((float) $coords[$i+2]) * -1;
					else
						$ycoord = (float) $coords[$i+2];

					$punto->setX( (float) $xcoord);
					$punto->setY( (float) $ycoord);
					$punto->setZ( (float) $zcoord);
					...
					if( $rotation < 45 || $rotation > 315 ){
						...
						if( ($pared == 2) && ($zMax < $picturePoint->getZ()) ){
							...
							$xMax = obtainMax( $this->coordenadas[$i][$this->caras[$i][$j]->getC1()], $this->coordenadas[$i][$this->caras[$i][$j]->getC2()], $this->coordenadas[$i][$this->caras[$i][$j]->getC3()], 2 );
								...
								$string =  "min: " . $xMin . "  max: " . $xMax . "  punto: " . $picturePoint->getX();
									$auxi[] = $string;	
						...
						if( ($pared == 1) && ($xMin > $picturePoint->getX()) ){
							...
							$string =  "min: " . $zMin . "  max: " . $zMax . "  punto: " . $picturePoint->getZ();
								$auxi[] = $string;
						...
						if( ($pared == 2) && ($zMin > $picturePoint->getZ()) ){
							...
							if( $picturePoint->getX() > $xMin && $picturePoint->getX() < $xMax ){
								...
								if( $dist < $minDist ){
									...
									$string =  "min: " . $xMin . "  max: " . $xMax . "  punto: " . $picturePoint->getZ();
									$auxi[] = $string;
